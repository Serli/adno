...
import NewJourney from "../Journeys/NewJourney/NewJourney";
...
class Editor extends Component {
    constructor(props) {
        ...
        this.state = {
            mobileMode: false,
            selected_project: JSON.parse(localStorage.getItem(this.props.match.params.id_project)),
            annotations: localStorage.getItem(this.props.match.params.id_journey) ? JSON.parse(localStorage.getItem(this.props.match.params.id_journey)).items : []
        }
    }
    ...
    componentDidMount() {
        ...
        if (!this.props.match.params.id_project || !checkIfProjectExists(this.props.match.params.id_project)) {
            this.props.history.push("/")
        } else {
            let selected_project = JSON.parse(localStorage.getItem(this.props.match.params.id_project))

            let tileSources;

            if (JSON.parse(localStorage.getItem(this.props.match.params.id_project)).manifest_url) {

                tileSources = [
                    JSON.parse(localStorage.getItem(this.props.match.params.id_project)).manifest_url
                ]

            } else {
                tileSources = {
                    type: 'image',
                    url: JSON.parse(localStorage.getItem(this.props.match.params.id_project)).img_url
                }
            }
            ...
            anno.on('createAnnotation', (annotation) => {
                ...
                var journeys_annotations = selected_journey.items
                ...
                if (!journeys_annotations) {
                    journeys_annotations = [
                        newAnnotation
                    ]
                } else {
                    journeys_annotations.push(newAnnotation)
                }
                ...
                selected_journey.items = journeys_annotations

                insertInLS(this.props.match.params.id_journey, JSON.stringify(selected_journey))

                this.setState({ annotations : journeys_annotations })

            });

            // Manage update of annotation
            anno.on('updateAnnotation', (upated_anno) => {
                let journey = JSON.parse(localStorage.getItem(this.props.match.params.id_journey))
                let annotations = journey.items
            ...
            <>
                <div className="left-bar">
                    <div className="mb-3">
                        ...
                        <input type="text" id="project_name" className="form-control" placeholder="Donnez un titre à votre projet" value={this.state.selected_project && this.state.selected_project.title} onChange={(e) => this.updateProjectName(e)} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="project_desc" className="form-label">Description</label>
                        <input id="project_desc" className="form-control" type="text" placeholder="Description de votre projet" value={this.state.selected_project && this.state.selected_project.description} onChange={(e) => this.updateProjectDesc(e)} />
                    </div>
                            ...
                            <div className="card">
                                <div className="card-body project-body">
                                    ...
                                    <div className="project-body-right">
                                        ...
                                        {/* <button id="preview" className="btn btn-primary subbutton" onClick={() => this.props.history.push("/project/" + this.props.match.params.id_project)}>Preview</button> */}
                                        <button id="preview" className="btn btn-primary subbutton" onClick={() => this.props.history.push( `/${this.props.match.params.id_project}/journeys/${this.props.match.params.id_journey}` )}>Preview</button>
                                    </div>
