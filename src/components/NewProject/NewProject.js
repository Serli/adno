    ...
    render() {
            ...
            if (this.state.projectName.length < 5) {
                ...
                if (get_url_extension(manifest_url) === "png" || get_url_extension(manifest_url) === "jpg" || get_url_extension(manifest_url) === "jpeg" || get_url_extension(manifest_url) === "json" || isUrlManifest["@type"] && isUrlManifest["@type"] === "sc:Manifest") {
                    ...
                    if (get_url_extension(manifest_url) === "png" || get_url_extension(manifest_url) === "jpg" || get_url_extension(manifest_url) === "jpeg") {
                        ...
                        let project = buildJsonProjectWithImg(projectID, this.state.projectName, document.getElementById("project_desc").value, new Date(), new Date(), manifest_url)
                        ...
                        this.props.history.push(`/${projectID}/journeys`)
                    } else {
                        fetch(manifest_url)
                            .then(rep => {
                                if (rep.status === 200) {
                                    ...
                                    rep.json().then(manifest => {
                                        ...
                                        if (manifest["@type"] && manifest["@type"] === "sc:Manifest") {
                                            ...
                                            project = buildJsonProjectWithManifest(projectID, this.state.projectName, document.getElementById("project_desc").value, new Date(), new Date(), resultLink)

                                        } else {
                                            project = buildJsonProjectWithManifest(projectID, this.state.projectName, document.getElementById("project_desc").value, new Date(), new Date(), manifest_url)
                                        }
        ...
        return (
            <form className="form-new-project">
                <div className="mb-3">
                    <label htmlFor="project_name" className="form-label" style={this.state.projectName.length >= 5 ? { color: "green" } : { color: "red" }}>Titre *</label>
                    <input id="project_name" value={this.state.projectName} onChange={(e) => this.setState({ projectName: e.target.value })} type="text" className={this.state.projectName.length >= 5 ? "form-control project_name_valid" : "form-control project_name_unvalid"} placeholder="Donnez un titre à votre projet" />
                </div>
                <div className="mb-3">
