...
export const buildYoutubeEmbed = (url, params) => {
  ...
  // let embed = `https://www.youtube.com/embed/${getYoutubeVideoID(url)}?autoplay=1`;
...
export const importProjectJsonFile = (loadedProject) => {
  ...
  fr.onload = function (e) {
    ...
    let journeys = imported_project.journeys

    let journeys_ids = []
    if(journeys.length > 0){
      journeys.forEach(journey => {
        journeys_ids.push(journey.id)
      })
    }
    ...
    let proj = {
      ...
      "journeys": journeys_ids
    }

    // we insert our imported project to our existing array of ADNO projects
    insertInLS("adno_projects", JSON.stringify(projects))
